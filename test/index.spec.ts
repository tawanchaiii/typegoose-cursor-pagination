...
describe("general functionality", () => {
  ...
  it("should return data in expected format", async () => {
    ...
    assert.equal(
      Object.prototype.hasOwnProperty.call(result, "totalDocs"),
      true
    );
    assert.equal(
      Object.prototype.hasOwnProperty.call(result, "previous"),
      true
    );
    assert.equal(
      Object.prototype.hasOwnProperty.call(result, "hasPrevious"),
      true
    );
...
describe("next/previous", () => {
  ...
  const query = {
    title: { $in: ["Post #1", "Post #2", "Post #3", "Post #4", "Post #5"] },
  };
  ...
  it("should return correct second page (on next)", async () => {
    const page1 = await Post.findPaged(baseOptions, query);
    const page2 = await Post.findPaged(
      { ...baseOptions, next: page1.next },
      query
    );
  ...
  it("should return correct second page (on next) for aggregation", async () => {
    const page1 = await Post.aggregatePaged(baseOptions, aggregatePipeline);
    const page2 = await Post.aggregatePaged(
      { ...baseOptions, next: page1.next },
      aggregatePipeline
    );
  ...
  it("should return correct third page (on next)", async () => {
    ...
    const page3 = await Post.findPaged(
      { ...baseOptions, next: page2.next },
      query
    );
  ...
  it("should return correct third page (on next) aggregation", async () => {
    ...
    const page3 = await Post.aggregatePaged(
      { ...baseOptions, next: page2.next },
      aggregatePipeline
    );
  ...
  it("should return correct second page (on previous)", async () => {
    ...
    const previousPage2 = await Post.findPaged(
      { ...baseOptions, previous: page3.previous },
      query
    );
  ...
  it("should return correct second page (on previous) aggregation", async () => {
    ...
    const previousPage2 = await Post.aggregatePaged(
      { ...baseOptions, previous: page3.previous },
      aggregatePipeline
    );
  ...
  it("should return correct first page (on previous)", async () => {
    ...
    const previousPage1 = await Post.findPaged(
      { ...baseOptions, previous: previousPage2.previous },
      query
    );
  ...
  it("should return correct first page (on previous) aggregation", async () => {
    ...
    const previousPage1 = await Post.aggregatePaged(
      { ...baseOptions, previous: previousPage2.previous },
      aggregatePipeline
    );
...
describe("query", () => {
  ...
  it("should allow aggregations", async () => {
    const result = await Post.aggregatePaged({}, [
      { $match: { title: { $in: ["Post #3", "Post #27"] } } },
    ]);
    assert.equal(result.docs.length, 2);
    assert.equal(result.docs[0].title, "Post #27");
    assert.equal(result.docs[1].title, "Post #3");
...
describe("explain", () => {
  it("should allow queryPlanner explains", async () => {
    const result = await Post.findPagedExplain({}, "queryPlanner", {
      title: { $in: ["Post #3", "Post #27"] },
    });
  ...
  it("should allow executionStats explains", async () => {
    const result = await Post.findPagedExplain({}, "executionStats", {
      title: { $in: ["Post #3", "Post #27"] },
    });
  ...
  it("should allow allPlansExecution explains", async () => {
    const result = await Post.findPagedExplain({}, "allPlansExecution", {
      title: { $in: ["Post #3", "Post #27"] },
    });
    assert.isTrue(!!result.queryPlanner);
    assert.isTrue(!!result.executionStats);
...
describe("population", () => {
  ...
  it("should allow populates with object", async () => {
    const result = await Post.findPaged({ limit: 1 }, {}, {}, [
...
describe("Plugin Options", () => {
  ...
  it("should set default limit when set", async () => {
    ...
    const ISBNShort = mongoose.model(
      "ISBNShort",
      ISBNShortSchema
    ) as IPaginateModel<any>;
